...
Claude Code 首版探测同时检查 PATH 和官方常见安装位置，返回当前主安装、全部候选安装、可执行文件路径、安装目录、版本及安装方式。保留全部候选是为了识别原生安装、WinGet、npm 和旧版 npm 并存造成的版本冲突；探测结果只存在于 TanStack Query 内存缓存，不写入 SQLite 或 `config.json`。
...
`is_last_attempt = 1` 的最终请求，并通过 `request_id` 关联 `usage_records`；因此日志
保留和清理策略同样决定会话观测数据的保留范围。

设置页提供显式的本地数据修复流程，由前端顺序编排四个细粒度 command：历史 OpenCode
会话归因回填、已捕获响应用量重解析、未知用量记录补齐、费用重算。会话回填只读取已保存的
...
- OpenCode 会话页按请求日志中的稳定会话 ID 聚合请求、Token、费用和失败数，并复用请求日志详情；
