...
账号保存后仍自动补齐对应的直接模型与聚合模型路由，但只有全局第一个渠道账号的新路由默认开启；后续新增的官方或自定义渠道账号，其新路由默认关闭，需要用户在模型服务页手动开启。已有路由的用户启停选择始终保留。
...
S3 仍是发现和离线回退通道。下载远端快照后，如 `lanPeer` 有效，客户端先用固定路径尝试
直连获取较新的完整快照；直连失败、身份不匹配、认证失败或返回版本不更新时继续导入原 S3
对象，不把正常 S3 刷新记为失败。移动会话页对已发现设备每 3 秒执行一次直连刷新并重新读取
本地导入区，因此新的会话状态和 OpenCode pending permission 无需等待 15 分钟 S3 周期。
...
每次切换压入系统返回历史。移动会话页读取桌面设备按上述规则筛选后的会话摘要，支持按共享设备
和运行状态筛选；最后交互随摘要存储并
...
command：历史 Claude Code / OpenCode 会话归因回填、已捕获响应用量重解析、未知用量记录补齐、
费用重算。会话回填只读取已保存的
