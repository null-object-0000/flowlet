...
| 默认 Flowlet 档位 | `LongCat-2.0 → pro + flash` | `v4-pro → pro`、`v4-flash → flash` | `kimi-k3 → pro`、`kimi-k2.7-code → pro` | `qwen3.8-max → pro`、`qwen3.7-max → pro`、`qwen3.7-flash → flash`、`qwen3.6-flash → flash`；Token Plan 账号为 `qwen3.8-max → pro`、`qwen3.6-flash → flash` | `glm-5.3 → pro`、`glm-5.3-flash → flash`、`glm-5.2 → pro`、`glm-4.7 → pro`、`glm-4.5-air → flash` | 天然支持全部白名单模型（`/models` 返回全部主流模型，用户显式勾选） | 无 |
