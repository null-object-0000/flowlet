...
> 该文件是 Codex CLI 启动时 serde 反序列化的**必填字段集合**：缺失 `shell_type` 等字段会直接报错
> （`missing field ...`）。推理档位只使用 `none` / `minimal` / `low` / `medium` / `high` / `xhigh`，
> 不使用 `max` / `ultra`（后者需要 Codex ≥ 0.144，0.137.0 会拒绝解析）。
