...
- 首次应用前备份 `$schema`、`model`、`small_model`、`server`、Provider 启停列表、`provider.flowlet` 和 `auth.json` 中的 `flowlet` 凭据；旧版备份在首次写入控制服务配置前会补录当时的 `server` 原值；
...
Flowlet 将 OpenCode 本地 Server 固定为 `127.0.0.1:4096`，只在本机回环地址监听。会话详情的“最近交互”会每 2 秒读取当前会话的待确认权限请求；用户选择“同意本次”时向 OpenCode 回复 `once`，选择“否决”时回复 `reject`。Flowlet 不提供永久放行操作，也不自行推断或代替用户确认。

控制接口属于 OpenCode 进程内状态，因此应用配置后必须重启 OpenCode。若端口被占用或 OpenCode 尚未启动，Flowlet 只显示控制服务未连接，不影响已有的只读会话与用量观测。
