...
交互 Demo 冻结为产品证据锚点，后续官网版式和文案调整不得反向修改 Demo 以迁就营销页面。
Demo 之后不再用五个同权重的大区块重复解释功能，而是收束为三段：

1. **工作流价值**：模型接入、故障定位、成本归因和本机任务四项实际收益，并紧接从 Agent 会话到
   真实请求、路由、上游结果和费用的完整证据链；
2. **支持与可信边界**：现有 Agent、模型渠道和实验性 Android 辅助端，与本地代理、响应零改写和
   用户自有同步能力放在同一区域说明；
3. **快速开始**：三步接入、下载/GitHub 操作和 Windows 11 / macOS / Linux 平台验证边界。

首屏高度也从接近整屏收紧为固定的紧凑价值区，让第二屏 Demo 更早进入视野。中英文使用相同信息
结构，移动端不出现横向溢出。
...
2026-08-11 在冻结交互 Demo 后完成官网其余内容重构：桌面端首屏高度实测为 `620px`，正文由原先
五段合并为三段；`1200 × 720` 中文页面与 `390 × 844` 英文页面均无横向溢出，导航锚点、深色价值
区、生态与本地边界组合区均完成浏览器检查，控制台无 warning/error；`npm run website:build` 与
`git diff --check` 通过。

构建过程中 `lottie-web` 的 direct `eval` 警告是现有第三方依赖警告，不应误报为本次共享化引入的新错误；但最终验收必须记录其真实输出。

## 13. 2026-08-11 官网效果图重构与 Demo 占位补齐

官网按 `preview.html` 效果图重新建立信息层级：

1. 首屏改为左侧产品价值主张、右侧 Agent → Flowlet → 模型渠道的本地请求工作流；
2. 增加三项核心价值和独立的工作原理管线，不再把所有价值说明塞进深色追溯区；
3. 保留完整的可交互桌面 Demo，并继续使用 `DesktopAppFrameView` 的 `1200 × 720` 固定产品画布；
4. 追溯区收敛为深色证据链和请求明细；
5. 本地优先、兼容范围和三步开始按效果图重新分段；
6. 导航增加“为什么 Flowlet”和“工作原理”锚点，中英文内容同步更新。

原先只有迁移提示的任务日志和应用设置已经填充 fixture Demo：

- `TaskLogsDemoView` 提供状态筛选、实时刷新、任务表格、选中态、进度和任务详情；
- `SettingsDemoView` 提供设置搜索、分类切换、通用偏好、数据捕获、同步、存储、维护和关于展示；
- 两个页面同时接入官网 `AppMockup` 和无后端的 `DesktopDemoApp`；
- 这些 Demo 只处理本地 React state 和静态 fixture，不调用真实 Tauri command，也不读写用户配置。

本轮没有修改 SQLite、Rust、Tauri command、`config.json`、代理协议或真实设置/任务逻辑。开发环境支持 Vite HMR；生产官网需要重新构建，代理无需重启。

浏览器验收中，桌面 Demo 外框 CSS 尺寸仍为 `1200 × 720`（边框内 `1198 × 718`）；任务日志和应用设置的共享内容区均为 `978 × 654`，横纵向 `scrollSize` 与 `clientSize` 一致。
