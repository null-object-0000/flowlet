...
会话数据，也不建立独立会话表；会话详情拆分为概览与最近交互，最近交互按需只读最后一次用户输入及此后全部原生回复、思考摘要和工具事件，已观测会话
仍可跳转到请求日志查看真实代理请求。
...
OpenCode 接入后通过仅监听本机的固定控制端口读取当前会话待确认权限；会话详情允许用户“同意本次”
或“否决”，操作直接回传 OpenCode。Flowlet 不暴露永久放行，控制服务不可用时也不根据本地数据库
推测等待确认状态。

Claude Code、OpenCode、ChatGPT（Codex）Desktop 和 Codex CLI 的原生会话详情均支持按需只读
最近交互。不同 Agent 的本地格式统一解析为用户消息、助手回复、思考摘要、工具调用、工具结果和
错误；界面定位最后一条用户消息，并展示它以及此后 Agent 产生的全部事件，不提供完整历史回溯。
内容不导入 Flowlet 数据库，并设置文件、事件数和单条内容上限。
