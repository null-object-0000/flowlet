...
会话数据，也不建立独立会话表；会话详情拆分为概览与最近交互，最近交互按需只读可读取范围内的完整原生时间线，按事件顺序混合展示用户输入、原生回复、思考摘要和工具事件，已观测会话
仍可跳转到请求日志查看真实代理请求。
...
最近交互。不同 Agent 的本地格式统一解析为用户消息、助手回复、思考摘要、工具调用、工具结果和
错误；界面在读取上限内按原生顺序渲染完整事件时间线，用户输入靠右，Agent 输出、思考和工具活动靠左；
思考摘要与工具活动保留在原时间位置并默认折叠，由用户按需展开。内容不导入 Flowlet 数据库，并设置文件、事件数和单条内容上限。

会话概览独立展示 Agent 原生累计 Token，最近交互事件展示可用的单次回复 Token 明细，包括输入、
输出、缓存读取、缓存写入和推理 Token。该数据不与 Flowlet 请求统计相加，避免同一请求重复计算。
