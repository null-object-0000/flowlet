...
- [x] 支持 Anthropic 请求头透传（X-Api-Key）
...
- [x] 支持 DeepSeek 模型列表同步（`/v1/models`）
- [x] 支持 DeepSeek 余额查询（`/user/balance`）
...
- [x] Claude Code 接入向导
- [x] 支持一键复制配置
- [x] 提供 `ANTHROPIC_BASE_URL=http://127.0.0.1:11434/anthropic` 配置提示
...
- [x] 关闭窗口时最小化到托盘而非退出
- [x] 开机自启动（tauri-plugin-autostart）
- [x] 托盘tooltip显示代理状态（运行中/已停止）

## 后续阶段：Docker / Web Console

- [x] Core 支持 headless 运行（`cargo run --bin headless`，独立于 Tauri GUI）
- [x] Web Console（嵌入式暗色主题仪表板，端口 8080，自动刷新）
- [x] Docker Compose（Dockerfile + docker-compose.yml，一键部署）
- [x] Volume 持久化（/data 目录挂载，SQLite 持久化）
- [x] 基础访问鉴权（Bearer Token 保护 API 端点）
...
- [x] 规则路由（按客户端/模型/协议匹配，强制路由到指定渠道账号）
- [x] 请求类型识别（chat/code/reasoning/long_context/tool_use 五类）
- [x] 小模型路由判断（短聊天请求自动使用渠道配置的便宜模型）
- [x] 成本/延迟/成功率综合调度（按账号评分排序候选，低成本高成功率优先）
