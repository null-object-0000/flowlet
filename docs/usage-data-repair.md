...
2. **Token 用量**：读取最终 attempt 的完整响应捕获，用当前解析器覆盖或补齐
   ...
   匹配唯一的原生消息级用量，歧义记录自动跳过；gzip / deflate / br 压缩响应在捕获层
   解压后重解析，新增或覆盖的记录沿用原始请求时间，不得写成执行维护的当前时间；
...
已在 LongCat 历史数据中观察到一种独立现象：响应 usage 提供了总输入、输出和总 Token，
但没有提供缓存命中与未命中的明细。此时总输入和输出仍是上游直接返回的真实来源，不能
把该请求视为 Token 未统计；未知的只是输入内部的缓存拆分。

2026-08-01 至 2026-08-02 的一次样本审计中，共发现 21 条此类 LongCat 请求。它们都来自
Claude Code 的 Anthropic Messages 调用，但同时包含 17 条流式 `flowlet-flash` 工具请求
和 4 条非流式 `flowlet-pro` 普通请求，并分布在多个会话中。同一请求形态的大多数其他
请求都能正常取得缓存拆分，因此现有证据不能将该现象归因于 SSE、工具调用、Claude Code
Auto mode、命令审批或固定模型档位。`tool_use` 只表示请求携带工具定义、工具选择或工具
历史，不能据此认定它是一次命令审批。

处理原则：

- 保留上游返回的 `input_tokens`、`output_tokens` 和 `total_tokens`；
- `input_cached_tokens` 与 `input_uncached_tokens` 继续保持未知，不填 0；
- 不将缺少缓存字段自动解释为“全部未命中”或“全部命中”；
- 不通过本地 tokenizer 自行拆分，也不从官网日汇总反推单条请求；
- 页面如需展示，应明确标注“上游未提供缓存明细”，并与真正未知总 Token 的记录区分。

### Token 计数请求与中断流

Anthropic `/messages/count_tokens` 是上下文长度计算接口，不执行模型生成。Flowlet 保留其
请求日志，但不会将它写成模型用量；捕获响应与 Agent 原生会话回填都会排除该路径，
“未知记录”维护阶段还会清理历史遗留的占位或误回填用量，不会删除原始请求日志。

新产生的流式请求如果在上游报错或下游取消前已经收到 usage，Flowlet 会保留上游原值：
存在终止标记或正数输出 usage 时记为 `complete`，只有起始输入 usage 时清空无法确认的
输出和总量并记为 `partial`。来源分别记录在 `usage_source`，部分记录不估算费用。客户端
取消后仍会终止上游读取，不会为了等待最终 usage 而在后台继续生成。
