...
export function AccountManagementSideSheet(props: Props) {
  ...
  const saveEditor = async (account: ChannelAccount, snapshot: AccountResourceSnapshotDraft | null) => {
    ...
    try {
      ...
      Toast.success("渠道账号与资源信息已保存，代理配置已热更新");
      closeEditor();
    } catch (error) {
      Toast.error(`保存失败：${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  const toggle = async (account: ChannelAccount, enabled: boolean) => {
    await save(accounts.map((item) => item.id === account.id ? { ...item, enabled } : item), enabled ? "账号已启用" : "账号已停用");
  };

  const remove = async () => {
    if (!deleteTarget) return;
    const saved = await save(accounts.filter((item) => item.id !== deleteTarget.id), "账号已删除");
  ...
  return (
    <>
      {request?.kind === "list" && editor == null ? <SideSheet
        ...
        motion={false}
        zIndex={1100}
        title={<div><strong>渠道账号管理</strong><small>共 {accounts.length} 个账号，{accounts.filter((item) => item.enabled).length} 个已启用</small></div>}
        width="min(720px, 94vw)"
        footer={(
          <Space style={{ justifyContent: "flex-end", width: "100%" }}>
            <Button onClick={closeManager}>关闭</Button>
            <Button icon={<IconPlus />} type="primary" theme="solid" disabled={!presets[0]} onClick={() => presets[0] && setEditor({ kind: "create", channelId: presets[0].id })}>新增账号</Button>
          </Space>
        )}
      ...
      >
        <div className={styles.body}>
          <div className={styles.toolbar}>
            <Input prefix={<IconSearch />} value={search} onChange={setSearch} placeholder="搜索账号名称或渠道" aria-label="搜索账号" />
            <Text type="tertiary">共 {filtered.length} 条</Text>
          </div>
          <div className={styles.list}>
            {filtered.length === 0 ? <div className={styles.empty}>{accounts.length ? `没有匹配「${search}」的账号` : "还没有配置渠道账号"}</div> : null}
              ...
              return (
                <div className={styles.row} key={account.id}>
                  <button type="button" className={styles.main} onClick={() => setEditor({ kind: "edit", account })}>
                    ...
                    <span className={styles.metrics}>{resourceDetails(account, snapshotByAccount.get(account.id)).map((item) => <span key={item.label}><em>{item.label}</em><b>{item.value}</b></span>)}</span>
                  </button>
                  <Tag color={status.color}>{status.label}</Tag>
                  <Switch checked={account.enabled} disabled={busy} aria-label={`${account.enabled ? "停用" : "启用"}账号 ${account.name}`} onChange={(checked) => void toggle(account, checked)} />
                  <Button icon={<IconMore />} theme="borderless" aria-label={`编辑账号 ${account.name}`} onClick={() => setEditor({ kind: "edit", account })} />
                  <Button icon={<IconDelete />} theme="borderless" type="danger" aria-label={`删除账号 ${account.name}`} onClick={() => setDeleteTarget(account)} />
                </div>
              );
            })}
          </div>
      ...
      /> : null}
      <Modal title="确认删除账号" visible={deleteTarget != null} zIndex={1200} footer={null} onCancel={() => setDeleteTarget(null)}>
        <Space vertical align="start" spacing="loose" style={{ width: "100%" }}>
          <Text>确定要删除账号「{deleteTarget?.name}」吗？删除后将退出所有路由，且无法恢复。</Text>
          <Space style={{ justifyContent: "flex-end", width: "100%" }}><Button onClick={() => setDeleteTarget(null)}>取消</Button><Button type="danger" theme="solid" loading={busy} onClick={() => void remove()}>确认删除</Button></Space>
        </Space>
      </Modal>
    </>
  );
...
function getStatus(account: ChannelAccount): { label: string; color: "green" | "red" | "grey" } {
...
function resourceDetails(account: ChannelAccount, snapshot?: AccountBalanceSnapshot) {
  const tokenPack = (account.resource_mode ?? (account.channel_id === "longcat" ? "token_pack" : "pay_as_you_go")) === "token_pack";
  if (!tokenPack) return [{ label: "余额", value: snapshot?.balance == null ? "-" : `${snapshot.balance} ${snapshot.currency ?? ""}`.trim() }];
  const rows = [{ label: "剩余", value: snapshot?.token_pack_remaining == null ? "-" : `${formatToken(snapshot.token_pack_remaining)} Tokens` }];
  if (snapshot?.token_pack_expire_at) rows.push({ label: "有效期", value: snapshot.token_pack_expire_at.split("T")[0] });
  return rows;
}
