...
import { buildModelServiceItems, type ModelServiceItem } from "./modelServiceView";
...
export function ModelServicesPage() {
  ...
  const toggleRoute = (route: RouteCandidate, enabled: boolean) => {
    actions.toggleExposedModel.mutate({ routes: routes.data ?? [], routeIds: [route.id], modelId: route.virtual_model_id, enabled });
  };
  ...
  return (
    <main className={styles.page}>
      <header className={styles.pageHeading}>
        ...
        <Button className={`${secondaryButtonStyles.button} ${secondaryButtonStyles.compact}`} type="tertiary" icon={<IconRefresh />} onClick={refresh} loading={loading}>{t("刷新模型")}</Button>
      </header>
        ...
        <section className={styles.listCard}>
          ...
          <div className={styles.modelList}>
            ...
            >
              ...
              <span>{t("{count} 条", { count: model.routes.filter((route) => route.enabled).length })}</span>
...
function ModelDetail({ model, accounts, channels, busy, onToggleRoute, t }: {
  ...
  busy: boolean;
  onToggleRoute: (route: RouteCandidate, enabled: boolean) => void;
  ...
  return <section className={styles.detailCard}>
    ...
    <div className={styles.detailBody}>
      <DetailSection title={t("基础配置")}>
        ...
        <div className={styles.configRow}><span>{t("支持协议")}</span><strong>{model.protocols.map((protocol) => protocol === "openai" ? "OpenAI" : "Anthropic").join(" / ") || "-"}</strong></div>
      </DetailSection>
      <DetailSection title={t("渠道路由")} note={t("数字越小优先级越高")}>
        {model.routes.map((route) => {
          ...
          return <div className={styles.routeRow} key={route.id}>
            <span className={styles.priority}>{route.priority + 1}</span>
            <span className={styles.routeCopy}><strong>{channelById.get(route.channel_id)?.name ?? route.channel_id} · {account?.name ?? route.account_id}</strong><small>{route.upstream_model} · {route.client_protocol === "openai" ? "OpenAI" : "Anthropic"}</small></span>
            <span className={usable ? styles.healthy : styles.unavailable}>{t(usable ? "可用" : "不可用")}</span>
            <Switch checked={route.enabled} disabled={busy} aria-label={t("启用路由 {name}", { name: route.upstream_model })} onChange={(checked) => onToggleRoute(route, checked)} />
          </div>;
        })}
      </DetailSection>
    </div>
