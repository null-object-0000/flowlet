...
import css from "./AccountEditorDrawer.module.css";
...
export function AccountEditorDrawer({
  ...
  return (
    <Drawer
      ...
      padding={0}
      zIndex={2000}
      classNames={{ root: "account-editor-drawer", header: css.drawerHeader, body: css.drawerBody, title: css.titleWrap }}
      title={
    ...
    >
      <div className={css.content}>
        <section className={`${css.section} ${css.sectionBasic}`}>
          <h3>基础信息</h3>
          <label><span className="account-key-label-row">选择渠道{request.mode === "edit" ? <small>（编辑时不可更改）</small> : null}</span></label>
          <div className={css.channelOptions}>
            {channels.map((item) => (
              <button
                type="button"
                key={item.id}
                disabled={request.mode === "edit"}
                className={`${css.channelOption}${draft.channel_id === item.id ? ` ${css.channelOptionSelected}` : ""}`}
        ...
        <section className={`${css.section} ${css.sectionResource}`}>
          {autoSyncBalance ? (
          ...
          {autoSyncBalance ? (
            <div className={css.resourceDetails}>
              <div className={css.resourceDetailsHeading}>
                <strong>按量付费信息</strong>
                <span className="sync-badge">自动同步</span>
              </div>
              <div className={css.resourceGrid}>
                <label className={css.balanceField}>
                  <span>账户余额</span>
            ...
            <>
              <div className={css.modeOptions}>
                <button
                  type="button"
                  className={`${css.mode}${resourceMode === "token_pack" ? ` ${css.modeSelected}` : ""}`}
                ...
                <button
                  type="button"
                  className={`${css.mode}${resourceMode === "pay_as_you_go" ? ` ${css.modeSelected}` : ""}`}
              ...
              {resourceMode === "token_pack" ? (
                ...
                <div className={css.resourceDetails}>
                  <div className={css.resourceDetailsHeading}><strong>按量付费信息</strong><span className="sync-badge warn">手动维护</span></div>
                  <div className={css.resourceGrid}>
        ...
        <section className={`${css.section} ${css.sectionAdvanced}`}>
          <button type="button" className={css.advancedToggle} onClick={() => setAdvancedOpen((value) => !value)}>
        ...
        {request.mode === "edit" ? (
          <section className={css.danger}><div><strong>删除账号</strong><span>删除后将退出所有路由，且无法恢复</span></div><Button variant="subtle" color="red" onClick={openRemoveConfirm}>删除</Button></section>
        ) : null}
