  ...
  return (
    <>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row.account_id}>
                    ...
                    <td title={row.last_error ?? ""}>{row.last_error ? row.last_error.length > 40 ? row.last_error.slice(0, 40) + "..." : row.last_error : "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <Panel>
        ...
        <p className="hint">综合调度算法：得分 = 0.4×归一化成本 + 0.3×归一化延迟 + 0.3×失败率。得分越低优先级越高。</p>
