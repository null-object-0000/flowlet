      ...
      <div className={styles.body}>
        ...
        <DetailSection title={t("会话信息")}>
          <div className={styles.detailGrid}>
            ...
            <DetailItem label={t("会话 ID")} value={session.sessionId} copyable wide onOpen={() => onViewRequestLogs(session.sessionId)} />
        ...
        <DetailSection title={t("活动时间")}>
          <div className={styles.detailGrid}>
            <DetailItem label={t("Flowlet 首次观测")} value={formatDate(session.startedAt, language)} />
            <DetailItem label={t("Flowlet 最近观测")} value={formatDate(session.updatedAt, language)} />
        ...
        <DetailSection title={t("Flowlet 请求统计")}>
          <div className={styles.metrics}>
            <Metric label={t("请求数")} value={session.requestCount.toLocaleString(language)} />
            <Metric label={t("成功")} value={session.successCount.toLocaleString(language)} />
            <Metric label={t("失败")} value={session.errorCount.toLocaleString(language)} warning={session.errorCount > 0} />
            <Metric label="Token" value={session.knownTokens.toLocaleString(language)} />
            <Metric label={t("费用")} value={`¥${session.estimatedCost.toFixed(4)}`} />
          </div>
        </DetailSection>
          ...
          {rows.map((row) => (
            <article className={styles.childRow} key={`${row.agentType}:${row.sessionId}`}>
              ...
              <div className={styles.childMeta}>
                <span>{formatShortDate(row.updatedAt, language)}</span>
                <small>{t("{requests} 次请求 · {tokens} Token · ¥{cost}", {
