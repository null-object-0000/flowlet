...
export function TaskLogsPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.toolbar} aria-label={t("任务筛选")}>
      ...
      <Select insetLabel={t("任务类型")} value={filter.jobType || "__all__"} optionList={[{ value: "__all__", label: t("全部类型") }, { value: "body-cleanup", label: t("Body 清理") }, { value: "agent-data-sync", label: t("Agent 数据同步") }, { value: "codex-account-sync", label: t("Codex 账号同步") }, { value: "channel-resource-sync", label: t("渠道资源自动同步") }, { value: "device-s3-sync", label: t("S3 设备同步") }, { value: "account-workspace-sync", label: t("渠道账号工作区同步") }, { value: "project-task-run", label: t("项目任务执行") }]} onChange={(value) => setFilter((current) => ({ ...current, jobType: value === "__all__" ? "" : String(value), page: 1 }))} />
      <Select style={{ width: "200px" }} insetLabel={t("触发方式")} value={filter.triggerSource || "__all__"} optionList={triggerOptions(t)} onChange={(value) => setFilter((current) => ({ ...current, triggerSource: value === "__all__" ? "" : String(value), page: 1 }))} />
...
function TaskDetail({ jobId, onClose }: { jobId: string | null; onClose: () => void }) {
  ...
  return <SideSheet title={sideTitle} visible={Boolean(jobId)} onCancel={onClose} width={DETAIL_SHEET_WIDTH} bodyStyle={{ padding: 0 }} zIndex={APP_OVERLAY_Z_INDEX.sideSheet} footer={detail.data?.job.status === "running" ? <Button type="danger" loading={cancel.isPending} disabled={detail.data.job.cancelRequested} onClick={() => void cancelTask()}>{detail.data.job.cancelRequested ? t("正在取消…") : t("取消任务")}</Button> : null}>
    ...
    {detail.data ? <div className={styles.detail}>
      ...
      {metrics ? <section><h4>{t("性能指标")}</h4><div className={styles.metrics}>{metric(t("总耗时"), metrics.durationMs)}{metrics.uploadedBytes != null ? metricText(t("上传数据"), formatBytes(metrics.uploadedBytes)) : null}{metrics.downloadedBytes != null ? metricText(t("下载数据"), formatBytes(metrics.downloadedBytes)) : null}{metric(t("跳过对象"), metrics.skippedObjects, false)}{metric(t("对象列举"), metrics.listMs)}{metric(t("对象下载"), metrics.downloadMs)}{metric(t("远端导入"), metrics.importMs)}{metric(t("生成快照"), metrics.snapshotMs)}{metric(t("快照上传"), metrics.uploadMs)}{metric(t("账号数量"), metrics.accounts, false)}{metric(t("同步成功"), metrics.syncedAccounts, false)}{metric(t("失效账号"), metrics.staleAccounts, false)}{metric(t("失败账号"), metrics.failedAccounts, false)}{metric(t("远端设备"), metrics.remoteDevices, false)}{metric(t("导入设备"), metrics.importedDevices, false)}{metric(t("导入天数"), metrics.importedDays, false)}{metric(t("未变更天数"), metrics.unchangedDays, false)}{metric(t("失败对象"), metrics.failedObjects, false)}{metric(t("目录版本"), metrics.revision, false)}{metric(t("本机关联账号"), metrics.linkedAccounts, false)}{metric(t("新增本机账号"), metrics.createdLocalAccounts, false)}{metric(t("目录扫描"), metrics.scanMs)}{metric(t("指纹比较"), metrics.compareMs)}{metric(t("会话解析"), metrics.parseMs)}{metric(t("数据库写入"), metrics.writeMs)}{metric(t("增量会话"), metrics.incrementalSessions, false)}{metric(t("全量会话"), metrics.fullSessions, false)}{metrics.sourceBytesProcessed != null ? metricText(t("读取数据"), formatBytes(metrics.sourceBytesProcessed)) : null}{metric(t("延后处理"), metrics.deferred, false)}</div></section> : null}
...
type SummaryMetrics = { durationMs?: number; uploadedBytes?: number; downloadedBytes?: number; skippedObjects?: number; listMs?: number; downloadMs?: number; importMs?: number; snapshotMs?: number; uploadMs?: number; accounts?: number; syncedAccounts?: number; staleAccounts?: number; failedAccounts?: number; remoteDevices?: number; importedDevices?: number; importedDays?: number; unchangedDays?: number; failedObjects?: number; revision?: number; linkedAccounts?: number; createdLocalAccounts?: number; scanMs?: number; compareMs?: number; parseMs?: number; writeMs?: number; deferred?: number; incrementalSessions?: number; fullSessions?: number; sourceBytesProcessed?: number };
...
function taskSubtitle(job: BackgroundJobRow, t: Translate) { const metrics = job.jobType === "device-s3-sync" ? parseSummary(job.summaryJson) : null; return metrics?.uploadedBytes != null && metrics.downloadedBytes != null ? `${t("上传数据")} ${formatBytes(metrics.uploadedBytes)} · ${t("下载数据")} ${formatBytes(metrics.downloadedBytes)}` : job.stage ?? "—"; }
